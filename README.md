...
By default that clone is a quick, shallow, single-commit fetch of
`LSPLANT_GIT_TAG=6.4` - LSPlant's last Maven-published release, confirmed
buildable on this exact toolchain (see [Build fix, part
4](#build-fix-part-4-the-search-itself-was-the-problem-2026-08-09) for why
this is the default now rather than the history-search described in
[part 2](#build-fix-part-2-from-date-guessing-to-actually-searching-history-2026-08-07)).
To pick a different exact ref, add a second entry to the `arguments` list
already in `native/build.gradle.kts`:
`arguments += listOf("-DANDROID_STL=c++_static", "-DLSPLANT_GIT_TAG=master")`
(or pass `-DLSPLANT_GIT_TAG=...` straight to `cmake` if you're driving it
directly rather than through Gradle). To instead re-enable the full
history search for the newest commit this toolchain can compile - useful
for checking whether LSPlant has landed anything better since - add
`-DLSPLANT_AUTO_SEARCH_HISTORY=ON` the same way; that ignores
`LSPLANT_GIT_TAG` and clones LSPlant's **full history** instead of a
shallow single commit, so it needs network access during configure and
...
- **LSPlant is pinned to `6.4` (Apr 2024) by default, which does NOT
  include Android 16 ART hook support** - see [Build fix, part
  4](#build-fix-part-4-the-search-itself-was-the-problem-2026-08-09) for
  the full reasoning. Three separate rounds of trying to auto-resolve a
  newer, still-buildable LSPlant commit each turned up a *different* way
  the attempt could fail (see parts 2 and 3 below too), which is why this
  no longer happens by default - it's an opt-in
  (`LSPLANT_AUTO_SEARCH_HISTORY=ON`, see [Building](#building)) for
  exploring what's currently buildable, not something to rely on for a
  production build. In practice this means: the native hooks in this
  module install and run fine on Android 16, but LSPlant's own
  ART-internals handling for Android 16 specifically may be incomplete or
  absent, which could mean anything from "works identically" to "hook
  installation itself fails" (logged, not a crash - see the bullet below)
  to, in the worst case, a hook installed against wrong assumptions. See
  [Is there a fundamentally different way to do
  this?](#is-there-a-fundamentally-different-way-to-do-this) for the
  longer-term options if you need current Android 16 coverage confirmed
  rather than assumed.
...
## Build fix, part 4: the search itself was the problem (2026-08-09)

### What the next CI run actually showed

With part 3's fix in place (checking `import x;` as well as `module x;`),
the search loop ran for real again - and this time its own commit-by-commit
trail, visible in the CI log, is worth reading directly:

```
-- LSPlant: commit 61e10e5 ... uses C++20 modules, trying an older commit
-- LSPlant: commit 74f4b1a ... uses C++20 modules, trying an older commit
   [... 60+ more commits, all rejected ...]
-- LSPlant: commit 16205e7 (Hook ReinitializeMethodsCode on A16+) uses C++20 modules, trying an older commit
   [... 20 more commits ...]
-- LSPlant: commit 2db8b66 (Modularize codes) uses C++20 modules, trying an older commit
-- LSPlant: commit 386200f (Run CI on Apple silicon) ... resolved
CMake Warning at CMakeLists.txt:295 (message):
  LSPlant: the resolved commit 386200f... is at or before
  LSPosed/LSPlant@16205e71aa3f028a729b00ed4aafd86793ea94f7 (2025-07-13, the
  first known Android-16 ART hook) - this build likely does NOT have
  working Android 16 support, even though it compiled successfully.
```

Two things stand out. First, the search walked straight through
`16205e7` - the Android-16 hook commit itself - still rejecting it as
"uses modules", and didn't stop until 20-odd commits *further back*, at
commits literally titled `Modularize codes`, `Modularize interfaces`,
`Use module for dex_builder`, `Use module for test`, `Use c++ 23`. Second,
even the commit it finally accepted then failed to compile for a **third,
different reason**:

```
common.hpp:3:10: fatal error: 'parallel_hashmap/phmap.h' file not found
    3 | #include <parallel_hashmap/phmap.h>
```

### Why this is a different kind of problem than parts 1-3

Parts 1-3 were each a specific, fixable gap in one regex. This is
structural. Two things this run proved, that no amount of regex-patching
addresses:

1. **LSPlant's C++20-modules adoption predates its Android-16 support.**
   The commits that introduce `export class`/`module`/`import` syntax
   sit *before* `16205e7` in the history, not after. That means
   "modules-free" and "has Android 16 ART hooks" are not both true
   anywhere in LSPlant's reachable history that this search can find -
   walking further back to escape one modules-using commit just walks
   past Android 16 support entirely, which is exactly what the
   `LSPLANT_RESOLVED_PREDATES_A16` warning caught, correctly, in real
   time.
2. **Vendoring LSPlant's source files without running LSPlant's own
   CMakeLists.txt means we don't know LSPlant's real dependency graph.**
   `parallel_hashmap` is a real, header-only dependency
   ([greg7mdp/parallel-hashmap](https://github.com/greg7mdp/parallel-hashmap),
   "just copy the parallel_hashmap directory to your project and you are
   good to go") that LSPlant's *own* build fetches for itself. We
   hand-glob `lsplant/*.cc` and hardcode a fixed list of include
   directories instead of using LSPlant's own build - which works fine
   for LSPlant's own code, but has no way to notice when LSPlant starts
   depending on something new. The module-declaration guard catches
   modules syntax specifically because that's the one failure mode it
   was written to look for; it was never going to catch a missing header
   it doesn't know exists.

Three CI rounds, three different failure shapes, each found by walking
further back through the same search. That's a pattern: this specific
search - "find the newest commit lacking C++20 module syntax" - was
answering a narrower question than "find the newest commit that actually
builds and has Android 16 support", and there is no guarantee the two
converge anywhere in reach.

### What changed

- **`LSPLANT_GIT_TAG` now defaults to `"6.4"`** instead of triggering a
  search. Confirmed real (`mvnrepository.com`, `central.sonatype.com`),
  confirmed buildable (that's what "last Maven-published release" means
  in practice), confirmed to predate all three failure classes above,
  because 6.4 is from April 2024 and every one of them was introduced
  well after.
- **The history search still exists, but is now opt-in**
  (`LSPLANT_AUTO_SEARCH_HISTORY=ON`), for deliberately checking what's
  currently buildable rather than as the default path. Its own warning
  check (does the resolved commit predate `16205e7`?) is unchanged, since
  it's still useful if someone does turn it on.
- **`parallel-hashmap` is now vendored defensively**, unconditionally,
  regardless of which LSPlant path is used - see the comment above
  `FetchContent_Declare(phmap_src ...)` in `CMakeLists.txt`. This doesn't
  fix the root cause (we still don't run LSPlant's own build, so we can
  still miss the *next* new dependency), but it closes this specific gap
  cheaply, and a header-only library costs almost nothing to vendor
  whether or not the code currently in use actually needs it.
- **An optional `flashable_module/post-fs-data.sh`** attempts a
  system-property-based complement to the native hooks - see [Is there a
  fundamentally different way to do
  this?](#is-there-a-fundamentally-different-way-to-do-this) below for
  what it does, its real scope, and the honest caveats on whether it even
  takes effect in time.

### Was switching away from LSPlant entertained seriously?

Yes, specifically as an alternative to more source-vendoring band-aids -
and ruled out with real data, not assumption. The two other ART-hooking
frameworks with any real adoption both stopped supporting new Android
versions years ago:
[asLody/SandHook](https://github.com/asLody/SandHook) - "support 4.4 -
11.0" - and [tiann/epic](https://github.com/tiann/epic) - "Supporting
5.0~11". Both cap out three major Android versions before this project's
target. Whatever LSPlant's build-toolchain friction costs, it's still the
only actively-maintained ART hook library in this space still adding
support for current Android releases at all (the `16205e7` commit this
whole investigation keeps returning to is literally that: new support,
recently added, still being maintained). Switching away from it to solve
a build problem would trade a build-time headache for a hard ceiling on
which Android versions can ever be supported - a strictly worse trade.
Full reasoning in [Is there a fundamentally different way to do
this?](#is-there-a-fundamentally-different-way-to-do-this) below.

### Sources consulted for this fix

| Claim | Source |
| --- | --- |
| Exact search-loop trail, the `parallel_hashmap` error, and the CMake `WARNING` text | `logs_84824011278.zip` and the preceding round's log (this project's own CI runs) |
| `16205e7` predates the modules-adoption commits in the search trail | Same CI log - the search loop's own `git checkout`/rejection order *is* dated evidence, since it walks strictly backward through real history |
| `parallel_hashmap` is real, header-only, and how to vendor it | [github.com/greg7mdp/parallel-hashmap](https://github.com/greg7mdp/parallel-hashmap) and its `README.md` |
| SandHook stops at Android 11 | [github.com/asLody/SandHook](https://github.com/asLody/SandHook) |
| Epic stops at Android 11 | [github.com/tiann/epic](https://github.com/tiann/epic) |
| Every `lsplant::Hook()` / `lsplant::Init()` call in `disable_hdr.cpp` already checks its return value and logs rather than crashes on failure | `disable_hdr.cpp`, read directly (lines around 214, 424, 477) |
...
- **Every real prior-art project doing something in this neighborhood does
  ...
  over-engineering.
- **A genuine, documented, exact-match SurfaceFlinger property exists -
  and it's now implemented as an optional extra layer, not just a
  research note.** `ro.surface_flinger.has_HDR_display` is real, current,
  and does exactly what the name suggests - confirmed by reading
  SurfaceFlinger's own sysprop schema directly from AOSP's current source:
  ["hwHDRDisplay indicates that the device has an High Dynamic Range
  display... Indicate support for this feature by setting
  ro.surface_flinger.has_HDR_display to true in
  device.mk"](https://chromium.googlesource.com/aosp/platform/frameworks/native/+/refs/heads/master/services/surfaceflinger/sysprop/SurfaceFlingerProperties.sysprop).
  `flashable_module/post-fs-data.sh` now attempts `resetprop
  ro.surface_flinger.has_HDR_display false` at early boot, fully optional
  and fails silently if `resetprop` isn't available. Two honest limits,
  documented at length in that file's own comments and worth repeating
  here: it's a `Readonly` (`ro.*`) property, so this only works if
  `resetprop` runs *before* SurfaceFlinger's own startup reads it - a
  closely related property test on a rooted Pixel 6 Pro for Android 13's
  "SDR dimming" feature [found no observable
  effect](https://www.esper.io/blog/android-sdr-dimming) despite flipping
  the documented flags, a concrete example of exactly this timing/gating
  risk - and even if it does take effect, it can only ever cover the
  `Display`/SurfaceFlinger detection path. It has no effect on apps that
  query `MediaCodecInfo` directly, which Google's own HDR playback
  documentation explicitly recommends as the standard pattern for any app
  not using ExoPlayer: check `MediaCodecList` for a decoder supporting the
  target HDR profile and go from there
  ([developer.android.com/media/grow/hdr-playback](https://developer.android.com/media/grow/hdr-playback)).
  That's precisely the codec-capability path this project's third hook
  (`MediaCodecInfo#getCapabilitiesForType()`) already exists to cover, and
  no system property changes that - the decoder HAL reports its own
  profile support regardless of what SurfaceFlinger believes about the
  display.

Bottom line on this question: no evidence turned up for a simpler,
reliable, *replacement* for app-level hooking - real prior art solving
adjacent problems converged on the same approach this project already
takes, and the one genuine system-wide lever that exists can, at best,
complement the `Display` half of the hooking already done here, not the
`MediaCodecInfo` half. That part of the architecture is very likely
correct as-is; `post-fs-data.sh` is worth keeping as free-if-it-works
defense in depth, not relied upon as a substitute.
...
- **Fall back to the last Maven release, `6.4`.** As of [part
  4](#build-fix-part-4-the-search-itself-was-the-problem-2026-08-09) this
  is no longer a fallback - it's the default. Three rounds of trying to
  auto-resolve something newer each found a new way to fail, converging
  toward this being the more honest choice: certain to build, explicit
  about not covering Android 16 ART internals, instead of a search that
  might quietly hand back something equally old (or broken in a new way)
  under the appearance of freshness.
- **Replace LSPlant with a different ART-hooking library.** Checked this
  round with actual data rather than left as a hypothetical: the two
  alternatives with any real adoption,
  [SandHook](https://github.com/asLody/SandHook) ("support 4.4 - 11.0")
  and [Epic](https://github.com/tiann/epic) ("Supporting 5.0~11"), both
  stopped supporting new Android versions years ago - switching to either
  would trade this project's build-toolchain friction for a hard ceiling
  three Android versions below the target, which is a worse problem than
  the one it would solve. This project already moved off YAHFA once for
  exactly this kind of gap (see [What
  changed](#what-changed-from-the-original)); the lesson generalizes.
  Ruled out, not just deprioritized.

Nothing here needs deciding today. The default (`6.4`) works and is
honest about its own limits; the search loop still exists behind
`LSPLANT_AUTO_SEARCH_HISTORY=ON` for checking whether LSPlant has landed
something better since; and the CMake-3.28 path remains the one option on
this list that actually ends the chase rather than managing it, whenever
it's worth the larger, dedicated effort.
